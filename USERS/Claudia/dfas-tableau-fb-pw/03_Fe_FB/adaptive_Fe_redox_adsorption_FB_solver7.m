function adaptive_Fe_redox_adsorption_FB_solver7(pHvals, FeT, PT, peINT, HFOsi, HFOwi)
% -------------------------------------------------------------------------
...
%% == (5) Plot Results (Alta Calidad + Fix Error IO) ==

% --- Helper interno para guardar ---
function export_high_res(hFit, name)
    % Ajustar tamaño de papel para que coincida con la pantalla
    set(hFit, 'PaperUnits', 'inches');
    set(hFit, 'PaperPosition', [0 0 12 7]); % 12x7 pulgadas (aprox 1200x700 px a 100dpi)
    
    % Imprimir usando driver pngcairo (clave para "bonito")
    print(hFit, name, "-dpngcairo", "-r150"); % r150 = 150 DPI (Alta def)
    close(hFit); % Cerrar inmediato para liberar memoria
    
    % Truco: Imprimir HTML para que Jupyter muestre la imagen generada
    % Esto evita usar 'imshow' y salta el error de ImageIO
    img_path = pwd(); 
    % Nota: en Jupyter a veces es relativo. Probamos ruta simple:
    printf(' <img src="%s" width="800"> \n', name);
end

fprintf('Generando gráficos de alta calidad...\n');
...
% FIGURA 1
h1 = figure(1, 'visible', 'off'); % Invisible para velocidad
plot(pHvals, xsolid_final_store(1,:), '-r', 'DisplayName', 'Fe(OH)3s'); hold on;
plot(pHvals, xsolid_final_store(2,:), '-b', 'DisplayName', 'FePO4(s)');
plot(pHvals, xsolid_final_store(3,:), '-g', 'DisplayName', 'Fe3(PO4)2(s)');
plot(pHvals, Fe3_aq_conc, '--m', 'DisplayName', 'Fe^{3+} (aq)');
plot(pHvals, HFO_P_ads, '-.k', 'DisplayName', 'HFO-P (ads)');
title('Distribución de Especies (Sólidos y Disueltos)');
xlabel('pH'); ylabel('Concentración (M)');
legend("Location", "northeast"); grid on; axis tight;
export_high_res(h1, "Fig1_Species.png");
...
title('Error de Balance de Masa');
xlabel('pH'); ylabel('Error Absoluto (M)');
legend("Location", "best"); grid on;
export_high_res(h2, "Fig2_MassError.png");

% FIGURA 3
h3 = figure(3, 'visible', 'off');
subplot(2,1,1);
plot(pHvals, massErrSiteS_store, '-o'); hold on;
plot(pHvals, massErrSiteW_store, '-s');
ylabel('Error Sitios'); title('Balance de Sitios Superficiales'); legend('Hfos','Hfow'); grid on;

subplot(2,1,2);
plot(pHvals, SiteT_s_store, '-'); hold on;
plot(pHvals, SiteT_w_store, '--');
ylabel('Sitios Totales'); xlabel('pH'); legend('Total S','Total W'); grid on;
export_high_res(h3, "Fig3_Sites.png");
...
semilogy(pHvals, max_error_store, '-k', 'LineWidth', 3);
xlabel('pH'); ylabel('Max Error (M)');
title('Error Máximo del Sistema'); grid on;
export_high_res(h4, "Fig4_MaxError.png");
