function adaptive_Fe_redox_adsorption_FB_solver6(pHvals, FeT, PT, peINT, HFOsi, HFOwi)
% -------------------------------------------------------------------------
...
if strcmp(graphics_toolkit(), "gnuplot")
  legend();                       % <-- sin Location (evita crash)
else
...
% --- Figure 3: Adsorption Site Errors and Totals (robust: no plotyy) ---
figure('Name', 'Adsorption Site Errors and Totals');
set(gcf, "color", "w");

subplot(2,1,1);
hold on; box on; grid on;
plot(pHvals, massErrSiteS_store, '-o', 'LineWidth', 1.5, 'DisplayName', 'Error Hfos');
plot(pHvals, massErrSiteW_store, '--s', 'LineWidth', 1.5, 'DisplayName', 'Error Hfow');
ylabel('Site balance error');
xlabel('pH');
if ~strcmp(graphics_toolkit(), "gnuplot")
  legend("Location","best");
else
  legend();   % sin Location para gnuplot
end
hold off;

subplot(2,1,2);
hold on; box on; grid on;
plot(pHvals, SiteT_s_store, '-^', 'LineWidth', 1.5, 'DisplayName', 'Site_s total');
plot(pHvals, SiteT_w_store, '--v', 'LineWidth', 1.5, 'DisplayName', 'Site_w total');
ylabel('Total sites');
xlabel('pH');
if ~strcmp(graphics_toolkit(), "gnuplot")
  legend("Location","best");
