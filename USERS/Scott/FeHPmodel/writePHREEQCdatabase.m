    ...
    if test(1)>=0;
        for j=1:length(test)
            if test(j)>0; Astr=num2str(Asoln(i,j));
                LSstr=[LSstr, " + ", Astr, cell2mat(components(j))];
            end
        end
    end
...
for i=1:size(Asolid,1)
    ...
    if test(1)<0; Astr=num2str(abs(Asolid(i,1)));
        LSstr=[LSstr," + ",Astr, " H+ "];
        RSstr=[RSstr, Astr, "H2O "];
    end
    test=Asolid(i,:);
    for j=1:length(test) % ignore H+ b/c taken care of already with test(1)<0 test
        %only negative will be hydroxide in normal tableau writing because use formation constants
